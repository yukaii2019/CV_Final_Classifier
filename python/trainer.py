...
class Trainer:
    ...
    ########################################################################  
        ...
        if args.train_val:
            self.train_loader = get_dataloader(args.dataset_dir, args.label_data, batch_size=args.batch_size, split='train_val')
        else:
    ...
    def train_epoch(self):
        ...
        logger.info(f'[{self.epoch + 1}/{self.args.num_epochs}] Train Loss: {total_loss:.5f} | Train Acc: {self.acc:.5f} | Train atnr: {self.atnr:.5f} | Train auc: {self.auc:.5f}')
